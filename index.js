...
function searchParamsURL(url) {
  ...
  let params = new Map();
  URLSearchParams.forEach((key, value) => params.set(value, key));
  return params;
...
function deleteSearchParams(keys, url) {
  ...
  for (let key of keys) {
    urlObj.searchParams.delete(key, [key]);
  }
  return urlObj.href;
...
function createURLWithParams(params, url) {
  ...
  for (let value in params) {
    urlObj.searchParams.append(value, params[value]);
  }
  return urlObj.href;
...
function modifyURLParameters(url, params) {
  let urlObj = new URL(url);
  for (let value in params) {
    if (urlObj.searchParams.has(value, params[value])) {
      urlObj.searchParams.set(value, params[value]);
    } else {
      urlObj.searchParams.append(value, params[value]);
    }
  }
...
function processUrl(url, options) {
  ...
  if (options.searchParams) {
    for (let value in options.searchParams) {
...
function getURLValues(url) {
  ...
  let arr = [];
  for (let key of keys) {
    arr.push(urlObj.searchParams.getAll(key));
  }
  return arr;
...
function getUrlKeys(url) {
  let urlObj = new URL(url);
  let arr = Array.from(urlObj.searchParams.keys());
  return arr;
  // Створюємо новий об'єкт URL з вхідної URL-адреси.
